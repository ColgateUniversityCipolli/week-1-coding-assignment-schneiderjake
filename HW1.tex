...
\hlcom{# see ?canvas_collatz for more input parameters of this function}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{knitrout}
\caption{Collatz Plot with seed 1313}
\label{CollatzPlot2}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\hlkwd{canvas_collatz}\hldef{(}
  ...
  \hlkwc{side} \hldef{=} \hlnum{FALSE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
\begin{alltt}
\hlkwd{canvas_collatz}\hldef{(}
\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"dark2"}\hldef{),}
\hlkwc{background} \hldef{=} \hlsng{"#fafafa"}\hldef{,}
\hlkwc{n} \hldef{=} \hlnum{242}\hldef{,}
\hlkwc{angle.even} \hldef{=} \hlnum{0.1313}\hldef{,}
\hlkwc{angle.odd} \hldef{=} \hlnum{0.024}\hldef{,}
\hlkwc{side} \hldef{=} \hlnum{FALSE}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\end{knitrout}
\caption{My own Custom COllatz Plot}
\label{CollatzPlot4}
\end{center}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
package for \texttt{R} \citep{aRtsy: Generative Art with 'ggplot2'}.
\end{enumerate}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(x} \hlkwb{<-} \hlkwd{seq}\hldef{(}\hlkwc{from}\hldef{=}\hlnum{0}\hldef{,} \hlkwc{to}\hldef{=}\hlnum{10}\hldef{,} \hlkwc{by}\hldef{=}\hlnum{1}\hldef{))}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(results} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{(x}\hlopt{+}\hlnum{1}\hldef{)}\hlopt{+}\hlnum{2}\hlopt{^}\hldef{(x}\hlopt{-}\hlnum{1}\hldef{))}
\end{alltt}
\begin{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(solution}\hlkwb{<-}\hlkwd{which}\hldef{(results}\hlopt{==}\hlnum{40}\hldef{))}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{(answer} \hlkwb{<-} \hldef{x[solution])}
\end{alltt}
\begin{verbatim}
## [1] 4
\end{verbatim}
...
\textbf{Solution:} The solution would fail in this case because the answer is not an integer and in order for our current approach to work the answer needs to be an integer as that is what we defined x as earlier. 
\end{enumerate}
\end{enumerate}
